...
docker_build() {
  ...
  if [[ -n $IMAGE_BUILD_ORIGIN ]]; then
    if grep -q "^ENV BITNAMI_CONTAINER_ORIGIN=" $IMAGE_BUILD_DIR/$DOCKERFILE; then
      sed -i 's/BITNAMI_CONTAINER_ORIGIN=.*/BITNAMI_CONTAINER_ORIGIN=GCR/' $IMAGE_BUILD_DIR/$DOCKERFILE
    else
